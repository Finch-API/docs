...
Assisted integrations enable Finch to offer industry-leading coverage and depth of data, while maintaining an API-first experience for developers. Automated and assisted integrations may also differ in [field support](docs/Integrations/Providers.md) based on the product and provider. 
...
Launch [Finch Connect](docs/Implementation-Guide/Connect/Set-Up-Finch-Connect.md) with **`manual=true`** so your user can see all providers that support assisted integrations. After granting your application access, you can retrieve an authorization **`code`** and exchange it for an **`access_token`** as usual.
