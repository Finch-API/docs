...
Follow [our full setup guide](docs/Integrations/Provider-specific-Information/Gusto) for more details.
...
Gusto's API returns SSN that are encrypted using `xsalsa20poly1305`, an authenticated encryption cipher. If you have been granted access to SSN data by Gusto, SSN will be supplied in the `encrypted_ssn` field. Gusto will provide you with a decryption key that only you can use to translate the encrypted value into its original format. For specific details and examples of how to decrypt SSNs, please refer to the [Gusto SSN Decryption Guide](docs/Integrations/Provider-specific-Information/Gusto-SSN-Decryption-Guide).
