...
2. Construct the correct [Finch Connect](/docs/Implementation-Guide/Connect/Set-Up-Finch-Connect) authorization URL again for your customer to reauthenticate. To create a more seamless experience, you can [bypass the employment system selection page](/docs/Implementation-Guide/Connect/Set-Up-Finch-Connect#selector-screen-bypass-flow) by using the `payroll_provider` parameter. You can find the `payroll_provider` of your customer by calling the [/introspect](https://developer.tryfinch.com/docs/reference/eee6e798b0f93-introspect) endpoint with their `access_token`.
3. Notify the customer that their connection is broken, and they can resolve it by reconnecting their account. To increase conversion, we recommend letting your customer know why reconnecting is beneficial to them and the services they will miss out in the interim. You can prompt your user to log on to their application dashboard where they can reconnect through [Finch Connect embedded](/docs/Implementation-Guide/Connect/Set-Up-Finch-Connect#embedded-finch-connect) in your app or send them an email with the [Finch Connect redirect](/docs/Implementation-Guide/Connect/Set-Up-Finch-Connect#redirect-to-finch-connect) link. 
...
A quick fix if you are hitting rate limit errors is to implement [batched requests](/docs/Implementation-Guide/Optimize/Batch-Requests). 
