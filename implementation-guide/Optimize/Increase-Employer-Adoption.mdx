...
You can programmatically add visual cues to show when the data import is complete. Finch provides a [List All Automated Jobs](/api-reference/4-management/list-automated-jobs) endpoint to monitor the status. Once the job’s `status` moves to `complete`, you can now call Finch’s various endpoints to read the data. A new job is created to sync the employer’s data 24 hours from the previous sync.
