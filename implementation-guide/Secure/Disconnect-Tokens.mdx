---
stoplight-id: onn9fhj6mn8mw
---

# Disconnect Tokens

A connection exists once an access token is obtained and [stored securely](/docs/Implementation-Guide/Secure/Store-Tokens.md). Once a connection is no longer needed or requested to be deleted by the customer, calling the [/disconnect](https://developer.tryfinch.com/docs/reference/c65ecbd512332-disconnect) endpoint will disconnect all associated access tokens for that particular connection. It is important to know how Finch defines a connection to understand what happens when a connection is disconnected. 
