...
Building upon the database tables `customers` & `finch_connections` defined in the [Manage Connections](/Implementation-Guide/Secure/Manage-Connections) guide, in order to implement multi-employer RBAC, we need a few additional tables.
