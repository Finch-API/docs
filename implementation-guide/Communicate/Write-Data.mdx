---
title: "Write Deductions & Contributions"
---

If you are already familiar with [Reading Data](/implementation-guide/Communicate/Read-Data) from the Finch `Orgainzation` and `Payroll` products, our `Deductions` product allows developers to write [contributions](/how-finch-works/unified-employment-api-glossary#contribution) and [deductions](/how-finch-works/unified-employment-api-glossary#deduction) across payroll providers for retirement, medical, and fringe benefits.
...
- [Automated](#automated-deductions) integrations support both the **reading** *and* **writing** of deductions and contributions.

- [Assisted](#assisted-deductions) integrations only allow the **writing** of deductions and contributions.

This information is available in [our provider documentation](/integrations/providers).
...
Our API allows you to both [create new benefits](/api-reference/3-deductions/create-benefit) in a payroll system and [get existing benefits](/api-reference/3-deductions/get-all-benefits) from the system. Both of these requests will respond with a `benefit_id` in the response body. For example, a creation request will respond with:
...
- [Create a New Benefit](/api-reference/3-deductions/create-benefit)
- [Enroll Individuals](/api-reference/3-deductions/enroll-individuals-in-benefits)
- [Unenroll Individuals](/api-reference/3-deductions/unenroll-individuals-from-benefits)
...
You can unenroll individuals by using the [`DELETE /benefits/{benefit_id}/individuals`](/api-reference/3-deductions/unenroll-individuals-from-benefits) endpoint. This will remove the enrollment configuration for an individual.
...
![automated-benefits.png](/images/automated-benefits-sequence-diagram.png)
...
![assisted-benefits.png](/images/assisted-benefts-sequence-diagram.png)
...
For assisted connections, it is important to submit payroll deductions to our API 7 days before the customer'spayroll close date. This will help ensure that changes can be processed within the current payroll period (unless explicitly specified in your API request via the `effective_date` request parameter). Otherwise, the change will be executed on the next payroll period. 
...
## Learn more
- [Deductions API](/products/benefits/Overview)
