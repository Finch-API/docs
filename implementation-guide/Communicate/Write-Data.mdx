...
If you are already familiar with [Reading Data](/Implementation-Guide/Communicate/Read-Data) from the Finch `Orgainzation` and `Payroll` products, our `Deductions` product allows developers to write [contributions](/Get-Started/Employment-API-Glossary#contribution) and [deductions](/Get-Started/Employment-API-Glossary#deduction) across payroll providers for retirement, medical, and fringe benefits.
