...
Finch returns Social Security Number (SSN) of an individual, which can be helpful in identifying unique individuals. However, SSN is not returned by default since it is a sensitive field; you must [enable SSN](/docs/Developer-Resources/Enable-SSN-Field.md) for your application first.
