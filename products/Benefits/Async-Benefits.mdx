---
stoplight-id: z9ssyh7davpgw
---

# Write Benefits Data Using Async APIs

The [Benefits](https://developer.tryfinch.com/docs/documentation/sk5k4fx5x9c7m-benefits) API allows developers to write [contributions](/docs/Get-Started/Employment-API-Glossary.md#contribution) and [deductions](/docs/Get-Started/Employment-API-Glossary.md#deduction) across payroll providers for retirement, medical, and fringe benefits using our unified API.
...
The types and features of each benefit can vary between payroll systems. For this reason, we provide a `benefits_support` field on the  [`/providers`](https://developer.tryfinch.com/docs/reference/1e18fe9cc1c67-providers) endpoint, which will return the available benefit types and features for each payroll provider. If you try to make a request using an access token that does not allow a certain configuration or benefit type, our API will respond with a 400 or 422 status code (see [Errors](https://developer.tryfinch.com/docs/reference/1f80003f67f0d-error-types)), depending on the specific error. This endpoint can help you avoid those errors by understanding beforehand what types of requests you can make.

This information is also available in [our provider documentation](/docs/Integrations/Providers.md).
...
You can unenroll individuals by using the [`DELETE /benefits/{benefit_id}/individuals`](https://developer.tryfinch.com/docs/reference/2bw7fbt8fztyx-unenroll-individuals-from-benefits) endpoint. This will remove the enrollment configuration for an individual.
...
## Learn more
- [Benefits API](/docs/Products/Benefits/Overview.md)
- [Supported Benefits Providers](/docs/Integrations/Providers.md)
