...
### Employer
An *employer* is an organization, company, or entity that hires and pays employees. An employer may consist of [one or many sub-entities](/docs/Implementation-Guide/Connect/Manage-Connections) and use one or many systems of record (i.e. [payroll providers](/docs/Integrations/Providers)) to manage and pay employees. Employers (or any sub-entities) can be referenced within the Finch APIs by their finch-issued `company_id`.
...
### Product
A *product* is a subgroup of Finch's standardized data model related to a particular segment or workflow of an employment system. Finch currently has three products: [Organization](/docs/Products/Organization/Overview), [Payroll](/docs/Products/Pay/Overview), and [Benefits](/docs/Products/Benefits/Overview).

### Connection
A *connection* is the link created between your application and the employer's provider through Finch. A connection is established after an employer authenticates via [Finch Connect](/docs/Get-Started/Finch-Connect). A connection is defined by a unique `application_id` + `company_id` + `payroll_provider_id` pairing. Therefore, an *employer* may have [one or many connections](/docs/Implementation-Guide/Connect/Manage-Connections) depending on if the employer is made up of sub-entities. 
...
### Access Token
An *access token* represents a single *connection* with an employer entity and allows the developer to query data from the associated employment system of the employer entity. You should treat access tokens with the same [level of security](/docs/Implementation-Guide/Secure/Store-Tokens) as you would passwords.
